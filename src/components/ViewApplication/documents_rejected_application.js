...
export default class DocumentsRejectedApplication extends React.Component {
	constructor(props) {
		super(props)
		this.state = {
			display: 'none',
	...
	componentWillMount() {
		...
		easygov.send(bootupsettings.ENDPOINTS.SEARCH_APPLICATIONS, { "searchTerm": term, "searchType": type, "status": "documents_rejected", "size":15, pageNumber:previousPage }, "documents_rejected", function (response, component) { })
		store.subscribe(() => {
			var response = store.getState()
			if (response.type === "documents_rejected" || response.type === "documents_rejected_tab") {
				if (response.code === 200) {
					data = response.data.objects
					pages = response.data.totalPages
					totalCount = response.data.totalCount
					store.dispatch({
						type: "REJECTED_APPLICATION_DATA",
						data: data
					})
					this.setState({
	...
	render() {
		...
		return (
			<div>
				{TablePopulator(data, "documents_rejected", 4, this, pages, "documents_rejected_tab", totalCount)}
			</div>
		)
	}
}
