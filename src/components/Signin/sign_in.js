...
export default class Signin extends Component {
  ...
  componentWillMount() {
    if (storage.getItemValue(keys.APP_PREFERENCE.IS_TOKEN_EXPIRED)) {
      this.setState({
        loginErrorMessage:
          "Your current session expired, Please login again to continue."
      });
    }
    store.subscribe(() => {
  ...
  callSigninApi() {
    if (userIdValue !== "" && passwordValue !== "") {
      network.sendLogin(
        bootupsettings.ENDPOINTS.SIGNIN,
        { email: userIdValue, password: passwordValue },
        "SIGNIN"
      );
    ...
    store.subscribe(() => {
      ...
      if (response.type === "SIGNIN") {
        ...
        storage.removeItemValue(keys.USER_PREFERENCE.IS_TOKEN_EXPIRED);
        storage.setCookies(
          keys.USER_PREFERENCE.USER_NAME,
          response.first_name ? response.first_name : "Admin"
        );
        window.location.href = "/dashboard";
      }
    });
      ...
      <div className="signin-container">
        ...
        <div className="signin-panel">
          ...
          <h1 className="home-title">
            T&P Crew <span className="heading-admin-title"> ADMIN</span>
          </h1>
          <div className="signin-textfield">
            <div className="userid-textfield-area">
              <div className="userid-icon">
                <i className="material-icons userid-icon-property">
                  perm_identity
                </i>
              </div>
          ...
          <div className="signin-bottom-link">
            ...
            <div className="signin-forget-password">
              <h5
                ...
                onClick={this.openForgetPassword.bind(this)}
              >
                Forget Password?
              </h5>
            </div>
