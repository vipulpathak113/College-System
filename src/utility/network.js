...
var token,email,password, languageCode = "en"
...
class network {
	...
	send_get_request(methodType, endpoint, actionType) {
		...
		if (storage.getItemValue(keys.USER_PREFERENCE.LANGUAGE)) {
			languageCode = JSON.parse(storage.getItemValue(keys.USER_PREFERENCE.LANGUAGE)).code
		}
		...
		$.ajax({
			...
			success: function (response) {
				NProgress.done()
				if (JSON.parse(storage.getItemValue(keys.USER_PREFERENCE.LANGUAGE)).code !== "en") {
	...
	send_file(endpoint, formdata, actionType) {
	    ...
	    contentType: 'application/json',
			...
			xhr: function () {  // custom xhr
				...
				if (_xhr.upload) { // check if upload property exists
